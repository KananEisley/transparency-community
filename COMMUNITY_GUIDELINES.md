# Community Guidelines

Welcome to the Transparency Platform community! These guidelines help ensure our community remains welcoming, productive, and focused on our mission of making government data more accessible.

## Our Mission

The Transparency Platform exists to democratize access to government data, empowering researchers, journalists, activists, and citizens to hold their governments accountable through better access to public information.

## Community Values

### 🤝 Respect and Inclusivity
- Treat all community members with respect and kindness
- Welcome newcomers and help them get started
- Embrace diverse perspectives and backgrounds
- Use inclusive language in all communications

### 🎯 Focus on Transparency
- Stay focused on government transparency and accountability
- Share knowledge and resources that benefit the community
- Respect the public nature of government data
- Promote ethical use of public information

### 🔍 Quality and Accuracy
- Provide accurate information and cite sources when possible
- Test features and report bugs thoroughly
- Give constructive feedback on suggestions and proposals
- Help maintain high standards for the platform

### 🚀 Collaboration
- Work together to improve the platform for everyone
- Share your expertise and learn from others
- Support community members in their transparency work
- Contribute to discussions in a meaningful way

## Communication Guidelines

### Issue Reports
When reporting bugs or requesting features:
- **Be specific**: Include detailed steps to reproduce issues
- **Be clear**: Use descriptive titles and clear descriptions
- **Be helpful**: Include screenshots, error messages, or logs when relevant
- **Be patient**: Allow time for review and response

### Discussions
In community discussions:
- **Stay on topic**: Keep discussions relevant to the platform and transparency
- **Be constructive**: Offer solutions, not just criticism
- **Be respectful**: Disagree with ideas, not people
- **Be concise**: Make your points clearly and efficiently

### Feature Requests
When suggesting new features:
- **Explain the use case**: Why is this feature needed?
- **Consider the audience**: How would this benefit the community?
- **Be realistic**: Consider technical and resource constraints
- **Research first**: Check if similar requests already exist

## Prohibited Behavior

The following behaviors are not acceptable in our community:

### ❌ Harassment and Abuse
- Personal attacks, insults, or discriminatory language
- Harassment based on identity, background, or beliefs
- Doxxing or sharing private information without consent
- Threats or intimidation of any kind

### ❌ Spam and Off-Topic Content
- Promotional content unrelated to government transparency
- Repetitive or low-quality posts
- Off-topic discussions that derail conversations
- Excessive cross-posting or duplicate issues

### ❌ Misuse of Platform
- Using the platform for illegal activities
- Attempting to access private or classified information
- Violating terms of service of underlying government APIs
- Sharing or requesting copyrighted material inappropriately

### ❌ Bad Faith Participation
- Trolling or deliberately disruptive behavior
- Spreading misinformation or conspiracy theories
- Refusing to engage constructively with feedback
- Creating multiple accounts to circumvent restrictions

## Licensing and Commercial Use

### Free vs. Premium Features
- Respect the distinction between free and premium features
- Don't attempt to circumvent licensing restrictions
- Direct licensing questions to appropriate channels
- Support the platform's sustainability through proper licensing

### Business Inquiries
- Use the license inquiry template for commercial questions
- Be transparent about your intended use case
- Respect intellectual property and licensing terms
- Work with the team on custom solutions when needed

## Moderation

### Community Moderation
Our community is moderated to ensure a positive environment:
- **Warnings**: First violations typically receive a warning
- **Temporary restrictions**: Repeated violations may result in temporary limits
- **Permanent bans**: Serious or persistent violations may result in permanent removal
- **Appeals**: Contact moderators if you believe action was taken in error

### Reporting Issues
To report community guideline violations:
- Use GitHub's reporting features for public issues
- Contact moderators directly for sensitive matters
- Provide specific examples and context
- Allow time for investigation and response

## Getting Help

### Technical Support
- **Free users**: Use GitHub issues and community discussions
- **Premium users**: Access priority support channels
- **Enterprise users**: Contact your dedicated support team

### Community Support
- Search existing issues and discussions before posting
- Use appropriate templates for different types of requests
- Tag relevant community members when appropriate
- Participate in community discussions to help others

## Contributing

### Ways to Contribute
- **Report bugs**: Help identify and fix issues
- **Suggest features**: Propose improvements and new capabilities
- **Help others**: Answer questions and provide support
- **Share knowledge**: Write tutorials, guides, or case studies
- **Spread the word**: Help grow the community responsibly

### Recognition
We recognize valuable community contributions through:
- **Contributor badges**: Recognition for helpful community members
- **Feature credits**: Acknowledgment for significant feature suggestions
- **Case studies**: Highlighting impactful uses of the platform
- **Community spotlights**: Featuring community members and their work

## Privacy and Data

### Personal Information
- Don't share personal information unnecessarily
- Respect others' privacy and confidentiality
- Be mindful of what you include in public posts
- Use private channels for sensitive discussions

### Government Data
- Remember that government data, while public, may contain personal information
- Use data responsibly and ethically
- Respect privacy even when information is technically public
- Follow journalistic and research ethics when applicable

## Updates to Guidelines

These guidelines may be updated periodically to reflect community growth and changing needs. Major changes will be announced to the community with opportunity for feedback.

## Contact

For questions about these guidelines or to report violations:
- **General questions**: [Community discussions](https://github.com/transparency-platform/transparency-community/discussions)
- **Guideline violations**: Use GitHub's reporting features
- **Private matters**: Contact repository maintainers directly

---

**Thank you for being part of the Transparency Platform community!**

*Together, we're making government data more accessible and democracy more transparent.* 